...
[NPS](https://github.com/nue-mic/nps) OpenWrt repository

[NPS](https://github.com/nue-mic/nps) 的 OpenWrt 仓库
...
2. Download the `.ipk` file from the [Release](https://github.com/nue-mic/nps-openwrt/releases) / 从[发布页面](https://github.com/nue-mic/nps-openwrt/releases)下载 `.ipk` 文件.
...
1. Edit `feeds.conf` and add the NPS source / 编辑 `feeds.conf` 文件，添加 NPS 源:
   ```bash
   echo "src-git nps https://github.com/nue-mic/nps-openwrt.git" >> feeds.conf
   ```
